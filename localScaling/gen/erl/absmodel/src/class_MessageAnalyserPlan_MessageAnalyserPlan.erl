...
implemented_interfaces() -> [ <<"Object">>, <<"MessageAnalyserPlan">> ].
...
'm_deploy'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        put(this, C:set_val_internal(get(this), 'ls_ls_MessageAnalyserInterface',[C:get_val_internal(get(this), 'ls_MessageAnalyserInterface') | C:get_val_internal(get(this), 'ls_ls_MessageAnalyserInterface')])),
         %% MessageAnalyserPlan.abs:52--52
        put(this, C:set_val_internal(get(this), 'ls_MessageAnalyserInterface',[])),
         %% MessageAnalyserPlan.abs:53--53
        put(this, C:set_val_internal(get(this), 'ls_ls_DeploymentComponent',[C:get_val_internal(get(this), 'ls_DeploymentComponent') | C:get_val_internal(get(this), 'ls_ls_DeploymentComponent')])),
         %% MessageAnalyserPlan.abs:54--54
        put(this, C:set_val_internal(get(this), 'ls_DeploymentComponent',[])),
         %% MessageAnalyserPlan.abs:55--55
        T_1 = (fun() -> case O of
            null -> throw(dataNullPointerException);
            Callee=#object{oid=Oid,cog=Cog} ->
                %% self-call
...
'm_deploy_aux'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        put(vars, (get(vars))#{'speedPatchMap' := m_ABS_StdLib_funs:f_put(Cog,maps:get('speedPatchMap', get(vars)),maps:get('c4_large_0', get(vars)),0,[O,DC| Stack])}),
         %% MessageAnalyserPlan.abs:74--74
        put(vars, (get(vars))#{'odefault___MessageAnalyser_0_c4_large_0' => object:new(cog:start(Cog,maps:get('c4_large_0', get(vars))),class_EmailProcessingPipelineArchitecture_MessageAnalyser,[C:get_val_internal(get(this), 'dbObj'),[]],Cog,[O,DC| Stack])}),
         %% MessageAnalyserPlan.abs:75--75
        put(vars, (get(vars))#{'coreRequired_odefault___MessageAnalyser_0_c4_large_0' => 1}),
         %% MessageAnalyserPlan.abs:76--76
        ...
        T_2 = (fun() -> case C:get_val_internal(get(this), 'messageAnalyser_LoadBalancerObj') of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_connectInstance',[maps:get('odefault___MessageAnalyser_0_c4_large_0', get(vars)),[]],#task_info{method= <<"connectInstance"/utf8>>},Cog, true),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)(),
        T_2,
        dataUnit
...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
         %% MessageAnalyserPlan.abs:93--106
        put(vars, (get(vars))#{'tmp1459807987' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% MessageAnalyserPlan.abs:93--106
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1459807987', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% MessageAnalyserPlan.abs:93--106
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1459807987', get(vars)),[O,DC| Stack])}),
                 %% MessageAnalyserPlan.abs:93--106
                put(vars, (get(vars))#{'tmp1459807987' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1459807987', get(vars)),[O,DC| Stack])}),
                 %% MessageAnalyserPlan.abs:94--94
                put(vars, (get(vars))#{'dc' => m_ABS_StdLib_funs:f_fst(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% MessageAnalyserPlan.abs:95--95
                put(vars, (get(vars))#{'coreUsed' => m_ABS_StdLib_funs:f_snd(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% MessageAnalyserPlan.abs:96--96
                put(vars, (get(vars))#{'tmp727265047' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% MessageAnalyserPlan.abs:96--96
                future:await(maps:get('tmp727265047', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% MessageAnalyserPlan.abs:96--96
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp727265047', get(vars)), Cog, [O,DC| Stack])}),
                 %% MessageAnalyserPlan.abs:97--100
                put(vars, (get(vars))#{'total_speed' => begin
                 ...
                 %% MessageAnalyserPlan.abs:101--101
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    true ->  %% MessageAnalyserPlan.abs:102--102
                    put(vars, (get(vars))#{'tmp948234328' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% MessageAnalyserPlan.abs:102--102
                    future:await(maps:get('tmp948234328', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% MessageAnalyserPlan.abs:102--102
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp948234328', get(vars)), Cog, [O,DC| Stack])}),
                     %% MessageAnalyserPlan.abs:103--103
                    put(vars, (get(vars))#{'speedPerCore' =>  rationals:rdiv(maps:get('total_speed', get(vars)),maps:get('totalCore', get(vars))) }),
                     %% MessageAnalyserPlan.abs:104--104
