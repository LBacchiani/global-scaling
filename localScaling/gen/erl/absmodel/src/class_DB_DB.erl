...
'm_insertMessageInformation'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_messageId_0,V_attachmentsNumber_0,Stack)->
 ...
 'attachmentsNumber' => V_attachmentsNumber_0 }),
    try
         %% db.abs:43--43
        put(this, C:set_val_internal(get(this), 'messageInfo',m_ABS_StdLib_funs:f_insert(Cog,C:get_val_internal(get(this), 'messageInfo'),{ dataPair,maps:get('messageId', get(vars)),{ dataPair,(3 + maps:get('attachmentsNumber', get(vars))) ,m_ABS_StdLib_funs:f_timeValue(Cog,m_ABS_StdLib_funs:f_now(Cog,[O,DC| Stack]),[O,DC| Stack])}},[O,DC| Stack]))),
        dataUnit
 ...
 %% db.abs:45
 %% db.abs:45
'm_insertResult'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_id_0,Stack)->
    C=(get(this))#state.class,
    put(vars, #{ 'this' => O,
 'id' => V_id_0 }),
    try
        ...
        case m_ABS_StdLib_funs:f_isJust(Cog,maps:get('maybe', get(vars)),[O,DC| Stack]) of
            true ->  %% db.abs:49--49
            put(vars, (get(vars))#{'numberOfActivityWaiting' := (m_ABS_StdLib_funs:f_fst(Cog,m_ABS_StdLib_funs:f_fromJust(Cog,maps:get('maybe', get(vars)),[O,DC| Stack]),[O,DC| Stack]) - 1) }),
             %% db.abs:50--50
            put(vars, (get(vars))#{'analysisTime' => m_ABS_StdLib_funs:f_snd(Cog,m_ABS_StdLib_funs:f_fromJust(Cog,maps:get('maybe', get(vars)),[O,DC| Stack]),[O,DC| Stack])}),
             %% db.abs:51--51
            put(this, C:set_val_internal(get(this), 'messageInfo',m_ABS_StdLib_funs:f_insert(Cog,C:get_val_internal(get(this), 'messageInfo'),{ dataPair,maps:get('id', get(vars)),{ dataPair,maps:get('numberOfActivityWaiting', get(vars)),maps:get('analysisTime', get(vars))}},[O,DC| Stack])));
            false ->         ok
        end,
         %% db.abs:53--53
 ...
 %% db.abs:56
 %% db.abs:56
'm_analysisTimeTick'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_messageId_0,Stack)->
    C=(get(this))#state.class,
    put(vars, #{ 'this' => O,
 'messageId' => V_messageId_0 }),
    try
        ...
        _:Exception:Stacktrace ->
            io:format(standard_error, "Uncaught ~s in method analysisTimeTick and no recovery block in class definition, killing object ~s~n", [builtin:toString(Cog, Exception), builtin:toString(Cog, O)]),
            io:format(standard_error, "stacktrace: ~tp~n", [Stacktrace]),
            object:die(O, Exception), exit(Exception)
    end.
 %% db.abs:66
