-module(class_Architecture_AttachmentsManager).
-include_lib("../include/abs_types.hrl").
-behaviour(object).
-export([get_val_internal/2,set_val_internal/3,init_internal/0,get_state_for_modelapi/1,implemented_interfaces/0,exported/0]).
-compile(export_all).

implemented_interfaces() -> [ <<"Object">>, <<"AttachmentsManagerInterface">> ].

exported() -> #{  }.

%% --- Internal state and low-level accessors

-record(state,{'class'=class_Architecture_AttachmentsManager,'imageAnalyserLoadBalancer'=null,'speed_res'=null,'mcl'=null}).
'init_internal'()->
    #state{}.

 %% architecture.abs:490
'get_val_internal'(#state{'imageAnalyserLoadBalancer'=G},'imageAnalyserLoadBalancer')->
    object:register_read('imageAnalyserLoadBalancer'),
    G;
 %% architecture.abs:491
'get_val_internal'(#state{'speed_res'=G},'speed_res')->
    object:register_read('speed_res'),
    G;
 %% architecture.abs:492
...
'set_val_internal'(S,'imageAnalyserLoadBalancer',V)->
    object:register_write('imageAnalyserLoadBalancer'),
...
'init'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},[P_imageAnalyserLoadBalancer,Stack])->
    ...
    put(this, C:set_val_internal(get(this),'imageAnalyserLoadBalancer',P_imageAnalyserLoadBalancer)),
     %% architecture.abs:491--491
    put(this, C:set_val_internal(get(this),'speed_res',5)),
     %% architecture.abs:492--492
...
%% --- Methods
 ...
 %% architecture.abs:494
 %% architecture.abs:494
'm_manageAttachments'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_messageId_0,V_balancer_0,Stack)->
    C=(get(this))#state.class,
    put(vars, #{ 'this' => O,
 'messageId' => V_messageId_0,
 'balancer' => V_balancer_0 }),
    try
         %% architecture.abs:495--495
        ...
        T_1,
         %% architecture.abs:497--497
        T_2 = cog:create_task(C:get_val_internal(get(this), 'imageAnalyserLoadBalancer'),'m_newRequest',[maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
        T_2,
        dataUnit
