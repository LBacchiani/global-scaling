...
'm_acquire'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp1355387800' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_acquireInstance',[O,[]],#task_info{method= <<"acquireInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
             %% abs/lang/abslang.abs:1145--1145
            future:await(maps:get('tmp1355387800', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1145--1145
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp1355387800', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
...
'm_release'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp214212829' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_releaseInstance',[O,[]],#task_info{method= <<"releaseInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
             %% abs/lang/abslang.abs:1152--1152
            future:await(maps:get('tmp214212829', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1152--1152
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp214212829', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
