-module(class_LocalMonitor_ScalingWrapperImpl).
...
-record(state,{'class'=class_LocalMonitor_ScalingWrapperImpl,'balancer'=null}).
'init_internal'()->
    #state{}.
...
'm_deploy'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_n_0,Stack)->
 ...
 'n' => V_n_0 }),
    try
         %% localMonitor.abs:56--56
        put(vars, (get(vars))#{'times' => maps:get('n', get(vars))}),
         %% localMonitor.abs:57--57
        []=(fun Loop ([])->
            ...
            true -> receive
                 ...
                 %% localMonitor.abs:58--58
                T_1 = cog:create_task(C:get_val_internal(get(this), 'balancer'),'m_deploy',[[]],#task_info{method= <<"deploy"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
                T_1,
                 %% localMonitor.abs:59--59
...
'm_undeploy'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_n_0,Stack)->
 ...
 'n' => V_n_0 }),
    try
         %% localMonitor.abs:64--64
        put(vars, (get(vars))#{'times' => maps:get('n', get(vars))}),
         %% localMonitor.abs:65--65
        []=(fun Loop ([])->
            case (not cmp:eq(maps:get('times', get(vars)),0)) of
            false -> [];
            true -> receive
                 ...
                 %% localMonitor.abs:66--66
                T_1 = cog:create_task(C:get_val_internal(get(this), 'balancer'),'m_undeploy',[[]],#task_info{method= <<"undeploy"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
                T_1,
                 %% localMonitor.abs:67--67
                put(vars, (get(vars))#{'times' := (maps:get('times', get(vars)) - 1) }),
            Loop([])  end end)
        ([]),
        dataUnit
