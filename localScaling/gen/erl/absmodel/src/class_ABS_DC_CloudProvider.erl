...
'm_prelaunchInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_d_0,Stack)->
 ...
 'd' => V_d_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp502852148' => cog:create_task(maps:get('result', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:892--892
        future:await(maps:get('tmp502852148', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:892--892
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp502852148', get(vars)), Cog, [O,DC| Stack])}),
...
'm_prelaunchInstanceNamed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instancename_0,Stack)->
 ...
 'instancename' => V_instancename_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1795569369' => cog:create_task(maps:get('dc', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:904--904
        future:await(maps:get('tmp1795569369', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:904--904
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp1795569369', get(vars)), Cog, [O,DC| Stack])}),
...
'm_acquireInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1669938422' => cog:create_task(maps:get('instance', get(vars)),'m_getProvider',[[]],#task_info{method= <<"getProvider"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:925--925
        future:await(maps:get('tmp1669938422', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:925--925
        put(vars, (get(vars))#{'cp' => future:get_blocking(maps:get('tmp1669938422', get(vars)), Cog, [O,DC| Stack])}),
...
'm_internalShutdownInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp2048186532' => cog:create_task(maps:get('instance', get(vars)),'m_getShutdownDuration',[[]],#task_info{method= <<"getShutdownDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:961--961
        future:await(maps:get('tmp2048186532', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:961--961
        put(vars, (get(vars))#{'shutdown_duration' => future:get_blocking(maps:get('tmp2048186532', get(vars)), Cog, [O,DC| Stack])}),
        ...
        put(vars, (get(vars))#{'tmp543634584' => cog:create_task(maps:get('instance', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:968--968
        future:await(maps:get('tmp543634584', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:968--968
        put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp543634584', get(vars)), Cog, [O,DC| Stack])}),
...
'm_getAccumulatedCost'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        put(vars, (get(vars))#{'tmp1187152896' => m_ABS_StdLib_funs:f_elements(Cog,C:get_val_internal(get(this), 'launchedInstances'),[O,DC| Stack])}),
         %% abs/lang/abslang.abs:975--978
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1187152896', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                 ...
                 %% abs/lang/abslang.abs:975--978
                put(vars, (get(vars))#{'dc' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1187152896', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:975--978
                put(vars, (get(vars))#{'tmp1187152896' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1187152896', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:976--976
                put(vars, (get(vars))#{'tmp2056089267' => cog:create_task(maps:get('dc', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% abs/lang/abslang.abs:976--976
                future:await(maps:get('tmp2056089267', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% abs/lang/abslang.abs:976--976
                put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp2056089267', get(vars)), Cog, [O,DC| Stack])}),
