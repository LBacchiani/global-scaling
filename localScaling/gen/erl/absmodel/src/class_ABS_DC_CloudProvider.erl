...
'm_prelaunchInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_d_0,Stack)->
 ...
 'd' => V_d_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp973087613' => cog:create_task(maps:get('result', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
         %% abs/lang/abslang.abs:922--922
        future:await(maps:get('tmp973087613', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:922--922
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp973087613', get(vars)), Cog, [O,DC| Stack])}),
...
'm_prelaunchInstanceNamed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instancename_0,Stack)->
 ...
 'instancename' => V_instancename_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp197690005' => cog:create_task(maps:get('dc', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
         %% abs/lang/abslang.abs:934--934
        future:await(maps:get('tmp197690005', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:934--934
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp197690005', get(vars)), Cog, [O,DC| Stack])}),
...
'm_acquireInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp874378099' => cog:create_task(maps:get('instance', get(vars)),'m_getProvider',[[]],#task_info{method= <<"getProvider"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
         %% abs/lang/abslang.abs:956--956
        future:await(maps:get('tmp874378099', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:956--956
        put(vars, (get(vars))#{'cp' => future:get_blocking(maps:get('tmp874378099', get(vars)), Cog, [O,DC| Stack])}),
...
'm_internalShutdownInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp2124338399' => cog:create_task(maps:get('instance', get(vars)),'m_getShutdownDuration',[[]],#task_info{method= <<"getShutdownDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
         %% abs/lang/abslang.abs:993--993
        future:await(maps:get('tmp2124338399', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:993--993
        put(vars, (get(vars))#{'shutdown_duration' => future:get_blocking(maps:get('tmp2124338399', get(vars)), Cog, [O,DC| Stack])}),
        ...
        put(vars, (get(vars))#{'tmp1161698234' => cog:create_task(maps:get('instance', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
         %% abs/lang/abslang.abs:1000--1000
        future:await(maps:get('tmp1161698234', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:1000--1000
        put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp1161698234', get(vars)), Cog, [O,DC| Stack])}),
...
'm_getAccumulatedCost'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        put(vars, (get(vars))#{'tmp2102280782' => m_ABS_StdLib_funs:f_elements(Cog,C:get_val_internal(get(this), 'launchedInstances'),[O,DC| Stack])}),
         %% abs/lang/abslang.abs:1007--1010
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp2102280782', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                 ...
                 %% abs/lang/abslang.abs:1007--1010
                put(vars, (get(vars))#{'dc' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp2102280782', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:1007--1010
                put(vars, (get(vars))#{'tmp2102280782' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp2102280782', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:1008--1008
                put(vars, (get(vars))#{'tmp179168674' => cog:create_task(maps:get('dc', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
                 %% abs/lang/abslang.abs:1008--1008
                future:await(maps:get('tmp179168674', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% abs/lang/abslang.abs:1008--1008
                put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp179168674', get(vars)), Cog, [O,DC| Stack])}),
