-module(class_Architecture_MessageAnalyser).
...
-record(state,{'class'=class_Architecture_MessageAnalyser,'db'=null,'speed_res'=null,'mcl'=null}).
'init_internal'()->
    #state{}.

 %% architecture.abs:662
'get_val_internal'(#state{'db'=G},'db')->
    object:register_read('db'),
    G;
 %% architecture.abs:663
'get_val_internal'(#state{'speed_res'=G},'speed_res')->
    object:register_read('speed_res'),
    G;
 %% architecture.abs:664
...
'init'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},[P_db,Stack])->
    ...
    put(this, C:set_val_internal(get(this),'db',P_db)),
     %% architecture.abs:663--663
    put(this, C:set_val_internal(get(this),'speed_res',5)),
     %% architecture.abs:664--664
...
%% --- Methods
 ...
 %% architecture.abs:666
 %% architecture.abs:666
'm_insertResult'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_id_0,V_balancer_0,Stack)->
    C=(get(this))#state.class,
    put(vars, #{ 'this' => O,
 'id' => V_id_0,
 'balancer' => V_balancer_0 }),
    try
         %% architecture.abs:667--667
        ...
        T_1,
         %% architecture.abs:669--669
        put(vars, (get(vars))#{'numberOfActivityWaiting' => (fun() -> case C:get_val_internal(get(this), 'db') of
            null -> throw(dataNullPointerException);
            Callee=#object{oid=Oid,cog=Cog} ->
                %% self-call
                Vars=get(vars),
                Result=C:'m_insertResult'(Callee,maps:get('id', get(vars)),[O,DC,Vars| Stack]),
                put(vars, Vars),
                Result;
            Callee=#object{oid=ObjRef,cog=Cog} ->
                %% cog-local call
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_insertResult',[maps:get('id', get(vars)),[]],#task_info{method= <<"insertResult"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)()}),
         %% architecture.abs:670--670
        case cmp:eq(maps:get('numberOfActivityWaiting', get(vars)),0) of
            true ->  %% architecture.abs:670--670
            T_2 = (fun() -> case C:get_val_internal(get(this), 'db') of
                null -> throw(dataNullPointerException);
                Callee=#object{oid=Oid,cog=Cog} ->
                    %% self-call
                    Vars=get(vars),
                    Result=C:'m_analysisTimeTick'(Callee,maps:get('id', get(vars)),[O,DC,Vars| Stack]),
                    put(vars, Vars),
                    Result;
                Callee=#object{oid=ObjRef,cog=Cog} ->
                    %% cog-local call
                    ...
                    put(task_info,(get(task_info))#task_info{this=Callee}),
                    T=object:get_class_from_state(get(this)), % it's the callee state already
                    Result=T:'m_analysisTimeTick'(Callee, V_messageId,[O,DC,Vars,State| Stack]),
                    cog:object_state_changed(Callee#object.cog, Callee, get(this)),
                    put(task_info,(get(task_info))#task_info{this=O}),
                    put(this, cog:get_object_state(Cog, O)),
                    put(vars, Vars),
                ...
                Callee ->
                    %% remote call
                    TempFuture = cog:create_task(Callee,'m_analysisTimeTick',[maps:get('id', get(vars)),[]],#task_info{method= <<"analysisTimeTick"/utf8>>},Cog),
                    future:get_blocking(TempFuture, Cog, [O,DC| Stack])
            end end)(),
            T_2;
            false ->         ok
