...
'm_prelaunchInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_d_0,Stack)->
 ...
 'd' => V_d_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp327561535' => cog:create_task(maps:get('result', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:893--893
        future:await(maps:get('tmp327561535', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:893--893
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp327561535', get(vars)), Cog, [O,DC| Stack])}),
...
'm_prelaunchInstanceNamed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instancename_0,Stack)->
 ...
 'instancename' => V_instancename_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1586636359' => cog:create_task(maps:get('dc', get(vars)),'m_getStartupDuration',[[]],#task_info{method= <<"getStartupDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:905--905
        future:await(maps:get('tmp1586636359', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:905--905
        put(vars, (get(vars))#{'startup_duration' => future:get_blocking(maps:get('tmp1586636359', get(vars)), Cog, [O,DC| Stack])}),
...
'm_acquireInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1128032955' => cog:create_task(maps:get('instance', get(vars)),'m_getProvider',[[]],#task_info{method= <<"getProvider"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:926--926
        future:await(maps:get('tmp1128032955', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:926--926
        put(vars, (get(vars))#{'cp' => future:get_blocking(maps:get('tmp1128032955', get(vars)), Cog, [O,DC| Stack])}),
...
'm_internalShutdownInstance'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_instance_0,Stack)->
 ...
 'instance' => V_instance_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp680466239' => cog:create_task(maps:get('instance', get(vars)),'m_getShutdownDuration',[[]],#task_info{method= <<"getShutdownDuration"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:962--962
        future:await(maps:get('tmp680466239', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:962--962
        put(vars, (get(vars))#{'shutdown_duration' => future:get_blocking(maps:get('tmp680466239', get(vars)), Cog, [O,DC| Stack])}),
        ...
        put(vars, (get(vars))#{'tmp170050776' => cog:create_task(maps:get('instance', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
         %% abs/lang/abslang.abs:969--969
        future:await(maps:get('tmp170050776', get(vars)), Cog, [O,DC| Stack]),
        ok,
         %% abs/lang/abslang.abs:969--969
        put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp170050776', get(vars)), Cog, [O,DC| Stack])}),
...
'm_getAccumulatedCost'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        put(vars, (get(vars))#{'tmp1792904767' => m_ABS_StdLib_funs:f_elements(Cog,C:get_val_internal(get(this), 'launchedInstances'),[O,DC| Stack])}),
         %% abs/lang/abslang.abs:976--979
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1792904767', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                 ...
                 %% abs/lang/abslang.abs:976--979
                put(vars, (get(vars))#{'dc' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1792904767', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:976--979
                put(vars, (get(vars))#{'tmp1792904767' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1792904767', get(vars)),[O,DC| Stack])}),
                 %% abs/lang/abslang.abs:977--977
                put(vars, (get(vars))#{'tmp1171094469' => cog:create_task(maps:get('dc', get(vars)),'m_getAccumulatedCost',[[]],#task_info{method= <<"getAccumulatedCost"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% abs/lang/abslang.abs:977--977
                future:await(maps:get('tmp1171094469', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% abs/lang/abslang.abs:977--977
                put(vars, (get(vars))#{'cost' => future:get_blocking(maps:get('tmp1171094469', get(vars)), Cog, [O,DC| Stack])}),
