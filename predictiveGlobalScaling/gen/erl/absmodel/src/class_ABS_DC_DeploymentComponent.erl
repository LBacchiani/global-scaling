...
'm_acquire'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp699017717' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_acquireInstance',[O,[]],#task_info{method= <<"acquireInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
             %% abs/lang/abslang.abs:1146--1146
            future:await(maps:get('tmp699017717', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1146--1146
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp699017717', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
...
'm_release'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp712257252' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_releaseInstance',[O,[]],#task_info{method= <<"releaseInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
             %% abs/lang/abslang.abs:1153--1153
            future:await(maps:get('tmp712257252', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1153--1153
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp712257252', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
