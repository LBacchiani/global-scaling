...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
         %% Scale3.abs:247--260
        put(vars, (get(vars))#{'tmp1046286588' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% Scale3.abs:247--260
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1046286588', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% Scale3.abs:247--260
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1046286588', get(vars)),[O,DC| Stack])}),
                 %% Scale3.abs:247--260
                put(vars, (get(vars))#{'tmp1046286588' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1046286588', get(vars)),[O,DC| Stack])}),
                 %% Scale3.abs:248--248
                ...
                put(vars, (get(vars))#{'coreUsed' => m_ABS_StdLib_funs:f_snd(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% Scale3.abs:250--250
                put(vars, (get(vars))#{'tmp1111777709' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% Scale3.abs:250--250
                future:await(maps:get('tmp1111777709', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% Scale3.abs:250--250
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp1111777709', get(vars)), Cog, [O,DC| Stack])}),
                 %% Scale3.abs:251--254
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp2093333949' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% Scale3.abs:256--256
                    future:await(maps:get('tmp2093333949', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% Scale3.abs:256--256
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp2093333949', get(vars)), Cog, [O,DC| Stack])}),
                     %% Scale3.abs:257--257
