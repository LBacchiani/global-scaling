 ...
 'dcs' => V_dcs_0 }),
    try
        ...
        put(vars, (get(vars))#{'cost' => 0}),
         %% code.abs:1136--1139
        put(vars, (get(vars))#{'tmp1643522944' => maps:get('dcs', get(vars))}),
         %% code.abs:1136--1139
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1643522944', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                 ...
                 %% code.abs:1136--1139
                put(vars, (get(vars))#{'dc' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1643522944', get(vars)),[O,DC| Stack])}),
                 %% code.abs:1136--1139
                put(vars, (get(vars))#{'tmp1643522944' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1643522944', get(vars)),[O,DC| Stack])}),
                 %% code.abs:1137--1137
