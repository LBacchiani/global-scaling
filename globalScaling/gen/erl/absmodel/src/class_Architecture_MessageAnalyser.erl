...
'init'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},[P_db,Stack])->
    ...
    put(this, C:set_val_internal(get(this),'db',P_db)),
     %% architecture.abs:460--460
    put(this, C:set_val_internal(get(this),'speed_res',5)),
     %% architecture.abs:461--461
 ...
 'balancer' => V_balancer_0 }),
    try
        ...
        put(vars, (get(vars))#{'numberOfActivityWaiting' => (fun() -> case C:get_val_internal(get(this), 'db') of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_insertResult',[maps:get('res', get(vars)),maps:get('id', get(vars)),[]],#task_info{method= <<"insertResult"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)()}),
         %% architecture.abs:467--467
        case cmp:eq(maps:get('numberOfActivityWaiting', get(vars)),0) of
            true ->  %% architecture.abs:467--467
            T_2 = (fun() -> case O of
                null -> throw(dataNullPointerException);
                Callee=#object{oid=Oid,cog=Cog} ->
                    %% self-call
                ...
                Callee ->
                    %% remote call
                    TempFuture = cog:create_task(Callee,'m_takeFinalDecision',[maps:get('id', get(vars)),[]],#task_info{method= <<"takeFinalDecision"/utf8>>},Cog),
                    future:get_blocking(TempFuture, Cog, [O,DC| Stack])
            end end)(),
            T_2;
            false ->         ok
 ...
 'messageId' => V_messageId_0 }),
    try
        ...
        put(vars, (get(vars))#{'finalResults' => (fun() -> case C:get_val_internal(get(this), 'db') of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_returnMessageResults',[maps:get('messageId', get(vars)),[]],#task_info{method= <<"returnMessageResults"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)()}),
        ...
        put(vars, (get(vars))#{'res' => (fun() -> case maps:get('finalResults', get(vars)) of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_getRes',[[]],#task_info{method= <<"getRes"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)()}),
