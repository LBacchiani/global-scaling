...
'f_iterate'(Cog=#cog{ref=CogRef},V_tcs_0,Stack)->
    begin
        case V_tcs_0 of
            ...
            _ -> io:format("No match for tcs at abs/lang/abslang.abs:1438~n"), 
...
"/utf8>>]);
            _ -> io:format("No match for FnApp(tc) at abs/lang/abslang.abs:1445~n"), 
            exit(dataPatternMatchFailException)
        end
    end, begin
...
"/utf8>>]);
            _ -> io:format("No match for FnApp(tc) at abs/lang/abslang.abs:1452~n"), 
            exit(dataPatternMatchFailException)
        end
    end]), begin
...
"/utf8>>]);
            _ -> io:format("No match for FnApp(tc) at abs/lang/abslang.abs:1456~n"), 
            exit(dataPatternMatchFailException)
        end
    end]), <<"</testcase>
...
'f_iterateTS'(Cog=#cog{ref=CogRef},V_tss_0,Stack)->
    begin
        case V_tss_0 of
            ...
            _ -> io:format("No match for tss at abs/lang/abslang.abs:1468~n"), 
            exit(dataPatternMatchFailException)
        end
    end.
...
'f_tcs'(Cog=#cog{ref=CogRef},V_data_0,Stack)->
    begin
        case V_data_0 of
            ...
            _ -> io:format("No match for data at abs/lang/abslang.abs:1433~n"), 
            exit(dataPatternMatchFailException)
        end
    end.
...
'f_result'(Cog=#cog{ref=CogRef},V_data_0,Stack)->
    begin
        case V_data_0 of
            ...
            _ -> io:format("No match for data at abs/lang/abslang.abs:1435~n"), 
            exit(dataPatternMatchFailException)
        end
    end.
