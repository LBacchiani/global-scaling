...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1915066647' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% BaseScale.abs:294--307
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1915066647', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% BaseScale.abs:294--307
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1915066647', get(vars)),[O,DC| Stack])}),
                 %% BaseScale.abs:294--307
                put(vars, (get(vars))#{'tmp1915066647' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1915066647', get(vars)),[O,DC| Stack])}),
                ...
                put(vars, (get(vars))#{'tmp1400116867' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% BaseScale.abs:297--297
                future:await(maps:get('tmp1400116867', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% BaseScale.abs:297--297
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp1400116867', get(vars)), Cog, [O,DC| Stack])}),
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp56702195' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% BaseScale.abs:303--303
                    future:await(maps:get('tmp56702195', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% BaseScale.abs:303--303
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp56702195', get(vars)), Cog, [O,DC| Stack])}),
