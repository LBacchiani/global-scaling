...
implemented_interfaces() -> [ <<"Object">>, <<"TextAnalyserInterface">> ].
...
'm_analyzeText'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_messageHeader_0,V_messageBody_0,V_messageId_0,Stack)->
 ...
 'messageId' => V_messageId_0 }),
    try
         %% architecture.abs:255--255
        put(vars, (get(vars))#{'refused' => false}),
         %% architecture.abs:256--256
        case (not cmp:eq(maps:get('messageHeader', get(vars)),<<""/utf8>>)) of
            true -> ok;
            false ->         ok
        end,
         %% architecture.abs:257--257
        put(vars, (get(vars))#{'sentimentAnalysisResults' => <<""/utf8>>}),
         %% architecture.abs:259--259
        put(vars, (get(vars))#{'res' => iolist_to_binary([iolist_to_binary([iolist_to_binary([iolist_to_binary([iolist_to_binary([<<"Results of TextAnalysis by TextAnalyser in "/utf8>>, builtin:toString(Cog,builtin:thisDC(Cog))]), <<" on: 
