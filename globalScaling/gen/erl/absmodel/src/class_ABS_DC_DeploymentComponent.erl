...
'm_acquire'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp1465333888' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_acquireInstance',[O,[]],#task_info{method= <<"acquireInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% abs/lang/abslang.abs:1179--1179
            future:await(maps:get('tmp1465333888', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1179--1179
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp1465333888', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
...
'm_release'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp1033155741' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_releaseInstance',[O,[]],#task_info{method= <<"releaseInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% abs/lang/abslang.abs:1186--1186
            future:await(maps:get('tmp1033155741', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1186--1186
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp1033155741', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
