-module(class_Architecture_VirusScanner_LoadBalancer).
-include_lib("../include/abs_types.hrl").
-behaviour(object).
-export([get_val_internal/2,set_val_internal/3,init_internal/0,get_state_for_modelapi/1,implemented_interfaces/0,exported/0]).
-compile(export_all).

implemented_interfaces() -> [ <<"VirusScanner_LoadBalancerInterface">>, <<"Object">> ].

exported() -> #{  }.

%% --- Internal state and low-level accessors

-record(state,{'class'=class_Architecture_VirusScanner_LoadBalancer,'db'=null,'instancesConnected'=null,'nextInstance'=null,'pending'=null,'buffer_size'=null}).
'init_internal'()->
    #state{}.

 %% architecture.abs:273
'get_val_internal'(#state{'db'=G},'db')->
    object:register_read('db'),
    G;
 %% architecture.abs:274
'get_val_internal'(#state{'instancesConnected'=G},'instancesConnected')->
    object:register_read('instancesConnected'),
    G;
 %% architecture.abs:275
'get_val_internal'(#state{'nextInstance'=G},'nextInstance')->
    object:register_read('nextInstance'),
    G;
 %% architecture.abs:276
'get_val_internal'(#state{'pending'=G},'pending')->
    object:register_read('pending'),
    G;
 %% architecture.abs:277
...
'init'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},[P_db,Stack])->
    ...
    put(this, C:set_val_internal(get(this),'nextInstance',-1)),
     %% architecture.abs:276--276
    put(this, C:set_val_internal(get(this),'pending',0)),
     %% architecture.abs:277--277
 ...
 'messageId' => V_messageId_0 }),
    try
        ...
        case cmp:lt(C:get_val_internal(get(this), 'pending'),C:get_val_internal(get(this), 'buffer_size')) of
            ...
            T_1 = cog:create_task(maps:get('selectedInstance', get(vars)),'m_scanAttachment',[maps:get('attachment', get(vars)),maps:get('messageId', get(vars)),O,[]],#task_info{method= <<"scanAttachment"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog),
            T_1;
            false ->          %% architecture.abs:293--293
