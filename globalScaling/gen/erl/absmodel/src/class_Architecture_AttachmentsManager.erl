...
'm_manageAttachments'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_attachment_0,V_messageId_0,V_balancer_0,Stack)->
 ...
 'balancer' => V_balancer_0 }),
    try
        ...
        T_2 = (fun() -> case C:get_val_internal(get(this), 'imageAnalyserLoadBalancer') of
            null -> throw(dataNullPointerException);
            Callee=#object{oid=Oid,cog=Cog} ->
                %% self-call
                Vars=get(vars),
            ...
            Callee=#object{oid=ObjRef,cog=Cog} ->
                %% cog-local call
                V_image = maps:get('attachment', get(vars)),
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_newRequest',[maps:get('attachment', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)(),
        T_2,
        dataUnit
