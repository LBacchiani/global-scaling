...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
         %% Scale2.abs:288--301
        put(vars, (get(vars))#{'tmp1612303947' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% Scale2.abs:288--301
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1612303947', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% Scale2.abs:288--301
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1612303947', get(vars)),[O,DC| Stack])}),
                 %% Scale2.abs:288--301
                put(vars, (get(vars))#{'tmp1612303947' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1612303947', get(vars)),[O,DC| Stack])}),
                 %% Scale2.abs:289--289
                ...
                put(vars, (get(vars))#{'coreUsed' => m_ABS_StdLib_funs:f_snd(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% Scale2.abs:291--291
                put(vars, (get(vars))#{'tmp1165313953' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% Scale2.abs:291--291
                future:await(maps:get('tmp1165313953', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% Scale2.abs:291--291
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp1165313953', get(vars)), Cog, [O,DC| Stack])}),
                 %% Scale2.abs:292--295
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp2036132277' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% Scale2.abs:297--297
                    future:await(maps:get('tmp2036132277', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% Scale2.abs:297--297
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp2036132277', get(vars)), Cog, [O,DC| Stack])}),
                     %% Scale2.abs:298--298
