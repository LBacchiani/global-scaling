...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
         %% Scale1.abs:237--250
        put(vars, (get(vars))#{'tmp862682532' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% Scale1.abs:237--250
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp862682532', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% Scale1.abs:237--250
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp862682532', get(vars)),[O,DC| Stack])}),
                 %% Scale1.abs:237--250
                put(vars, (get(vars))#{'tmp862682532' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp862682532', get(vars)),[O,DC| Stack])}),
                 %% Scale1.abs:238--238
                ...
                put(vars, (get(vars))#{'coreUsed' => m_ABS_StdLib_funs:f_snd(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% Scale1.abs:240--240
                put(vars, (get(vars))#{'tmp545144829' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% Scale1.abs:240--240
                future:await(maps:get('tmp545144829', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% Scale1.abs:240--240
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp545144829', get(vars)), Cog, [O,DC| Stack])}),
                 %% Scale1.abs:241--244
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp707940781' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% Scale1.abs:246--246
                    future:await(maps:get('tmp707940781', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% Scale1.abs:246--246
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp707940781', get(vars)), Cog, [O,DC| Stack])}),
                     %% Scale1.abs:247--247
