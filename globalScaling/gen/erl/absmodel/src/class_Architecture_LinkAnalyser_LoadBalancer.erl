...
-record(state,{'class'=class_Architecture_LinkAnalyser_LoadBalancer,'db'=null,'instancesConnected'=null,'nextInstance'=null,'buffer_size'=null}).
'init_internal'()->
    #state{}.
 ...
 'messageId' => V_messageId_0 }),
    try
        ...
        T_1 = (fun() -> case maps:get('selectedInstance', get(vars)) of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_analyzeLink',[maps:get('links', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"analyzeLink"/utf8>>},Cog),
