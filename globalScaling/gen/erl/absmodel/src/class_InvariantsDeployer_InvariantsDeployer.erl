...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
        ...
        put(vars, (get(vars))#{'tmp1842916393' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% InvariantsDeployer.abs:267--280
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1842916393', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% InvariantsDeployer.abs:267--280
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1842916393', get(vars)),[O,DC| Stack])}),
                 %% InvariantsDeployer.abs:267--280
                put(vars, (get(vars))#{'tmp1842916393' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1842916393', get(vars)),[O,DC| Stack])}),
                ...
                put(vars, (get(vars))#{'tmp1387766753' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% InvariantsDeployer.abs:270--270
                future:await(maps:get('tmp1387766753', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% InvariantsDeployer.abs:270--270
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp1387766753', get(vars)), Cog, [O,DC| Stack])}),
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp1406008073' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% InvariantsDeployer.abs:276--276
                    future:await(maps:get('tmp1406008073', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% InvariantsDeployer.abs:276--276
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp1406008073', get(vars)), Cog, [O,DC| Stack])}),
