 ...
 'id' => V_id_0 }),
    try
        ...
        []=(fun Loop ([])->
            ...
            true -> receive
                ...
                put(vars, (get(vars))#{'links' := m_ABS_StdLib_funs:f_insertElement(Cog,maps:get('links', get(vars)),iolist_to_binary([iolist_to_binary([maps:get('mailData', get(vars)), <<"_Link"/utf8>>]), builtin:toString(Cog,maps:get('i', get(vars)))]),[O,DC| Stack])}),
                 %% architecture.abs:120--120
                put(vars, (get(vars))#{'i' := (maps:get('i', get(vars)) + 1) }),
            Loop([])  end end)
        ([]),
         %% architecture.abs:122--122
        put(vars, (get(vars))#{'i' := 0}),
        ...
        T_3 = (fun() -> case C:get_val_internal(get(this), 'headerAnalyserLoadBalancer') of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_newRequest',[maps:get('headers', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)(),
        T_3,
         %% architecture.abs:131--131
        case cmp:gt(maps:get('n_links', get(vars)),0) of
            true ->  %% architecture.abs:131--131
            T_4 = (fun() -> case C:get_val_internal(get(this), 'linkAnalyserLoadBalancer') of
                ...
                Callee ->
                    %% remote call
                    TempFuture = cog:create_task(Callee,'m_newRequest',[maps:get('links', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>},Cog),
                    future:get_blocking(TempFuture, Cog, [O,DC| Stack])
            end end)(),
            T_4;
            false ->          %% architecture.abs:132--132
        T_4 = (fun() -> case C:get_val_internal(get(this), 'messageAnalyserLoadBalancer') of
            ...
            Callee ->
                %% remote call
                TempFuture = cog:create_task(Callee,'m_insertResult',[<<""/utf8>>,maps:get('messageId', get(vars)),[]],#task_info{method= <<"insertResult"/utf8>>},Cog),
                future:get_blocking(TempFuture, Cog, [O,DC| Stack])
        end end)(),
        T_4
        end,
        ...
        case ((not cmp:eq(maps:get('messageHeader', get(vars)),<<""/utf8>>))) or ((not cmp:eq(maps:get('messageBody', get(vars)),<<""/utf8>>))) of
            ...
            T_5 = (fun() -> case C:get_val_internal(get(this), 'textAnalyserLoadBalancer') of
                ...
                Callee ->
                    %% remote call
                    TempFuture = cog:create_task(Callee,'m_newRequest',[maps:get('messageHeader', get(vars)),maps:get('messageBody', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>},Cog),
                    future:get_blocking(TempFuture, Cog, [O,DC| Stack])
            end end)(),
            T_5;
            false ->          %% architecture.abs:134--134
        ...
        []=(fun Loop ([])->
            ...
            true -> receive
                ...
                T_6 = (fun() -> case C:get_val_internal(get(this), 'virusScannerLoadBalancer') of
                    ...
                    Callee ->
                        %% remote call
                        TempFuture = cog:create_task(Callee,'m_newRequest',[maps:get('attach', get(vars)),maps:get('messageId', get(vars)),[]],#task_info{method= <<"newRequest"/utf8>>},Cog),
                        future:get_blocking(TempFuture, Cog, [O,DC| Stack])
                end end)(),
                T_6,
            Loop([])  end end)
