...
'm_patchSpeed'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},V_dcs_0,Stack)->
 ...
 'dcs' => V_dcs_0 }),
    try
         %% Scale4.abs:295--308
        put(vars, (get(vars))#{'tmp921239294' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('dcs', get(vars)),[O,DC| Stack])}),
         %% Scale4.abs:295--308
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp921239294', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% Scale4.abs:295--308
                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp921239294', get(vars)),[O,DC| Stack])}),
                 %% Scale4.abs:295--308
                put(vars, (get(vars))#{'tmp921239294' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp921239294', get(vars)),[O,DC| Stack])}),
                 %% Scale4.abs:296--296
                ...
                put(vars, (get(vars))#{'coreUsed' => m_ABS_StdLib_funs:f_snd(Cog,maps:get('entry', get(vars)),[O,DC| Stack])}),
                 %% Scale4.abs:298--298
                put(vars, (get(vars))#{'tmp2133604062' => cog:create_task(maps:get('dc', get(vars)),'m_total',[dataSpeed,[]],#task_info{method= <<"total"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                 %% Scale4.abs:298--298
                future:await(maps:get('tmp2133604062', get(vars)), Cog, [O,DC| Stack]),
                ok,
                 %% Scale4.abs:298--298
                put(vars, (get(vars))#{'speed_aux' => future:get_blocking(maps:get('tmp2133604062', get(vars)), Cog, [O,DC| Stack])}),
                 %% Scale4.abs:299--302
                ...
                case (not cmp:eq(maps:get('total_speed', get(vars)),-1)) of
                    ...
                    put(vars, (get(vars))#{'tmp1638099407' => cog:create_task(maps:get('dc', get(vars)),'m_getNumberOfCores',[[]],#task_info{method= <<"getNumberOfCores"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog,true)}),
                     %% Scale4.abs:304--304
                    future:await(maps:get('tmp1638099407', get(vars)), Cog, [O,DC| Stack]),
                    ok,
                     %% Scale4.abs:304--304
                    put(vars, (get(vars))#{'totalCore' => future:get_blocking(maps:get('tmp1638099407', get(vars)), Cog, [O,DC| Stack])}),
                     %% Scale4.abs:305--305
